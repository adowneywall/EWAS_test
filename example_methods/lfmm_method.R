...
library(fdrtool)
fdr<-fdrtool(z.score,statistic = c("normal"),verbose = F,plot = F) 
hist(fdr$qval)
hist(fdr$pval)
fdr$param
...
b.ord<-which(simu$B[rev(order(simu$B[,1])),]>0)
points(-log10(p.values.calibrated)[b.ord]~b.ord,
       col="red",pch=16)
...
plot(-log10(fdr$pval)~c(1:1000),xlab="Loci Position")

b.ord<-which(simu$B[rev(order(simu$B[,1])),]>0)
points(-log10(fdr$pval)[simu$causal]~simu$causal,
       col="red",pch=16)

points(-log10(fdr$pval)[which(-log10(fdr$pval) > 3)]~which(-log10(fdr$pval) > 3),
       col="blue",pch=21,cex=2)
abline(h = 3,col="red")
