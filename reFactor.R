refactor <- function(data_file, k, data_fromGithub = FALSE, data_raw=FALSE,
                     covarfile = NULL, covar_fromGithub = FALSE, numcomp = NULL, stdth = 0.02, out = "refactor") {
  ...
  if(data_raw == TRUE){
    ...
    if(data_fromGithub == TRUE){
      O = as.matrix(read.csv(text=getURL(data_file),
                             header=F,sep = ""))
    }else{O = as.matrix(read.table(data_file))}
  ...
  O = O[include,]
  if(data_raw == FALSE){cpgnames = cpgnames[include]}
  ...
  {
    if(covar_fromGithub == TRUE){
      covs = as.matrix(read.csv(text=getURL(covarfile),
                             header=F,sep = ""))
    }else{covs = as.matrix(read.table(covarfile))}
    sample_id2 <- covs[, 1]
